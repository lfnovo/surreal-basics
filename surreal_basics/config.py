...
def _parse_surreal_url() -> Optional[Tuple[str, int, Literal["http", "ws"]]]:
    """Parse SURREAL_URL if set, returning (host, port, mode) or None."""
...
class SurrealConfig:
    ...
    mode: Literal["http", "ws"] = field(default_factory=_get_mode)
...
def init(
    ...
    mode: Optional[Literal["http", "ws"]] = None,
    ...
    Args:
        ...
        mode: Connection mode - "http" or "ws" (default: env SURREAL_MODE or "ws")
...
def set_mode(mode: Literal["http", "ws"]) -> None:
    """Set the connection mode."""
    get_config().mode = mode


def get_mode() -> Literal["http", "ws"]:
